...
const OnlyLocalStoragesToDos = () => {
  ...
  const addLocalMemo = (e: React.FormEvent<HTMLFormElement>) => {
    const start = new Date();
    e.preventDefault();
    ...
    const time = end.getMilliseconds() - start.getMilliseconds();
    console.log("addLocalMemo time:", time);
  };

  const deleteLocalMemo = (idx: number) => {
    const start = new Date();
    let newMemo = [...memo];
    ...
    const time = end.getMilliseconds() - start.getMilliseconds();
    console.log("deleteLocalMemo time:", time);
  };

  return (
    <section>
      <h1>Local Storage ToDos</h1>
      <MemoInputForm onSubmit={addLocalMemo}>
        <input
          value={text}
...
export default OnlyLocalStoragesToDos;
